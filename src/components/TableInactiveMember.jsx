...
import { fetchUsers, fetchTrxs, fetchLaba, fetchKodeOperators, fetchModuleTrxs, fetchSaldoMembers, fetchInactiveMembers } from '../store/actionCreators';
...
const TableInactiveMember = () => {
    ...
    useEffect(() => {
        dispatch(fetchInactiveMembers());

        let total = 0;
        inactive?.data_member_inactive?.forEach(item => {
            total += item.Saldo;
        });
        setTotalSaldo(total);
    }, [dispatch]);
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-fixed shadow-md sm:rounded-lg">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '300px' }}>Owner</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '200px' }}>Alamat</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '200px' }}>Saldo</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '200px' }}>Hilang</th>
                        </tr>
                    </thead>
                    <tbody>
                        {inactive?.data_member_inactive?.map((s, index) => {
                            return (
                                <tr key={index}>
                                    <td className='text-center' style={{ color: 'black' }}>{index + 1}</td>
                                    <td className='text-center' style={{ color: 'black' }}>{s.ID}</td>
                                    <td className='text-center' style={{ color: 'black' }}>{s.Nama}</td>
                                    <td className='text-center' style={{ color: 'black' }}>{s.Owner}</td>
                                    <td className='text-center' style={{ color: 'black' }}>{s.Alamat}</td>
                                    <td className='text-center' style={{ color: 'black' }}>{formatNumber(s.Saldo)}</td>
                                    <td className='text-center' style={{ color: 'black' }}>{s.Hilang}</td>
                                </tr>
                            );
                        })}
                        <tr>
...
export default TableInactiveMember;
