...
import { fetchKodeOperators, setPickedJenis } from '../store/actionCreators';

const DropdownJenis = () => {

  const dispatch = useDispatch();

  const operators = ['Data', 'Reguler', 'SMS', 'Telpon', 'Transfer', 'Lainnya'];
  const pickedJenis = useSelector((state) => state.jenis.jenis);
  ...
  const handleJenisClick = (jenis) => {
    dispatch(setPickedJenis(jenis));
    localStorage.setItem('pickedJenisLocal', jenis);
    setIsOpen(false);
  };

  useEffect(() => {
    const storedPickedJenis = localStorage.getItem('pickedJenisLocal');
    if (storedPickedJenis) {
      dispatch(setPickedJenis(storedPickedJenis));
    }
  }, [dispatch]);
  return (
    <div className="relative inline-block text-left mb-2 mt-2 mr-2 ml-2">
      ...
      >
        <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
          <li>
            <span className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" onClick={() => handleJenisClick('Semua Jenis')}>
              Semua Jenis
            </span>
          </li>
          {operators.map((operator, index) => (
            <li key={index}>
              <a
                ...
                onClick={() => handleJenisClick(operator)}
              >
                {operator}
              </a>
            </li>
