...
import { fetchUsers, fetchTrxs, fetchLaba, fetchKodeOperators, fetchModuleTrxs, fetchSaldoMembers, fetchInactiveMembers, fetchDepositMembers } from '../store/actionCreators';
...
const TableMemberDeposit = () => {
    ...
    const depositMembers = useSelector((state) => state.depositMembers.depositMembers);
    ...
    useEffect(() => {
        dispatch(fetchDepositMembers());

        let total = 0;
        depositMembers?.data_member_inactive?.forEach(item => {
            total += item.Jumlah_Deposit;
        });
        setTotalSaldo(total);
    }, [dispatch]);


    console.log(depositMembers, 'depoooo');
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-fixed shadow-md sm:rounded-lg">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '300px' }}>ID</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '500px' }}>Nama</th>
                            <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ width: '500px' }}>Owner</th>
                        </tr>
                    </thead>
                    <tbody>
                        {depositMembers?.data_member_inactive?.map((s, index) => {
                            return (
                                <tr key={index}>
                                    <td className='text-center' style={{ color: 'black' }}>{index + 1}</td>
                                    <td className='text-center' style={{ color: 'black' }}>{s.ID}</td>
                                    <td className='text-center' style={{ color: 'black' }}>{formatNumber(s.Jumlah_Deposit)}</td>
...
export default TableMemberDeposit;
